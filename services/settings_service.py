...
class SettingsService:
    ...
    def get_row(self, key: str) -> AppSetting:
        ...
        return AppSetting(key=key, value=model().model_dump())
    ...
    def update(
        ...
        row = self._repo.upsert(key, validated.model_dump(), updated_by=updated_by)
        self._s.commit()
        return row
    ...
    def get_default_location_code(self) -> str:
        row = self._repo.get(SettingKey.DEFAULT_LOCATION_CODE.value)
        if row is None:
