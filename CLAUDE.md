...
`scripts/generate_year_spans.py` is the offline counterpart to the two admin endpoints above, for regenerating the *whole* configured year span (the `seed_year_range` setting, 2021–2030 by default) in one run without going through HTTP or an admin session. Santhigiri event occurrences are regenerated through a real `SanthigiriEventService` (mirroring the wiring in `api/deps.py`), driving its `generate_all_occurrences_streaming` generator directly against `DATABASE_URL` and printing the same per-year progress a client would read from the NDJSON stream. Base panchangam data is generated differently: rather than the plain day-by-day `PanchangamGenerationService.generate_streaming` the live `/generate` endpoint uses, the script calls `core/calendar/panchangam.py::get_panchangam_data_range` directly — a range-batched computation (one chunked `find_discrete` pass for Thithi/Nakshatra transitions, one pass each for Kollavarsham/Chandra Masa/sunrise-sunset instead of recomputing each per day) worth paying for on a full multi-year regeneration but not exposed on the live streaming endpoint or its UI. `get_panchangam_data_range` lives alongside `get_panchangam_data()` in `core/calendar/panchangam.py` as an additive, script-only entry point — it does not change what the `/generate` endpoint or the generation UI do. Run the script locally (`python scripts/generate_year_spans.py`, optionally with `--start-year`/`--end-year`/`--location`/`--skip-panchangam`/`--skip-events`) or manually via the `Generate year spans` GitHub Actions workflow (`.github/workflows/generate-year-spans.yml`, `workflow_dispatch` only — it needs a `DATABASE_URL` secret pointed at the target database).
