...
└── app/
    ...
    │   ├── sql/                    # Standalone bootstrap schema + seed SQL applied to a brand-new Neon/Postgres via psql
    │   ├── alembic/                # Alembic env.py + versions/ — schema migrations for db/models/ (see "Migrations" below)
...
Schema changes to `db/models/` are applied as **Alembic** migrations — `app/db/alembic/` (`env.py` + `versions/`), driven by `alembic.ini` at the repo root. `env.py` resolves `DATABASE_URL` the same way `db/database.py` does (same `postgres://` -> `postgresql://` normalisation) and imports `db/models/` so `SQLModel.metadata` is the autogenerate target — there is no `sqlalchemy.url` hardcoded in `alembic.ini`. Full workflow, including how to bring an already-bootstrapped database (Neon/production, or anything stood up from `db/sql/01_schema.sql`/`02_seed.sql`) under Alembic without re-running its DDL, is in `app/db/alembic/README.md`.

`0001_baseline_schema` is a snapshot of the schema as it stood the moment Alembic was introduced — equivalent to `db/sql/01_schema.sql` plus every pre-Alembic hand-written migration under `db/sql/migrations/` (0001-0006). Its `upgrade()` only runs for a brand-new, empty database; an already-bootstrapped one is `alembic stamp`ped onto it instead (see `app/db/alembic/README.md`). `db/sql/01_schema.sql`/`02_seed.sql` remain the one-time bootstrap snapshot for standing up a database from scratch and are not regenerated when a migration is added; `db/sql/migrations/` is now historical and is not added to — new schema changes get an Alembic revision instead.
...
| `alembic` | Schema migrations for `db/models/` — `app/db/alembic/`, driven by the repo-root `alembic.ini` (see "Migrations" above) |
...
`DATABASE_URL` must be set (in the environment or a local `.env`) or startup fails fast — it points at a Neon/Postgres database. Startup only ensures the schema exists (`init_db()`); it does not load any data. Seed a **brand-new** database once by applying `db/sql/01_schema.sql` and `db/sql/02_seed.sql` with `psql` (10 years of pre-computed data, 2021–2030). See `db/sql/README.md`. Alternatively, run `alembic upgrade head` to create the schema instead of `01_schema.sql` — see `app/db/alembic/README.md` — then seed data separately; an **already-bootstrapped** database (including existing Neon/production) should be brought under Alembic with `alembic stamp` rather than `upgrade`, per that same README.
