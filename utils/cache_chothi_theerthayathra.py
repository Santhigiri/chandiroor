...
def calculate_chothi_theerthayathra_for_year(yearly_data: PanchangamCache, year: int)-> List[date]:
    occurances = []
    ...
    for transition in sorted_transitions:
        ...
        if transition.end_time - yearly_data[transition.end_time.date()].sunrise > timedelta(hours=3):
...
def update_chothi_theerthayathra(panchangamCache: PanchangamCache)-> PanchangamCache:
    """
    ...
    for year in range(start_year.year, end_year.year + 1):
        ...
        chothi_dates = calculate_chothi_theerthayathra_for_year(yearly_data, year)
...
def cache_chothi_theerthayathra():
    cache: PanchangamCache = load_cache()
    updated_cache = update_chothi_theerthayathra(cache)
