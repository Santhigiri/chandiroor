...
async def lifespan(app: FastAPI):
    ...
    # Defensive safety net only — Alembic (`alembic upgrade head`, run once
    # per deploy in .github/workflows/docker-build-push.yml, not by this
    # process; see db/sql/README.md) is the authoritative way schema changes
    # reach a database. create_all() only creates *missing* tables and never
    # alters an existing one, so it's a harmless no-op once migrations have
    # run; it exists so a local `uvicorn --reload` dev flow that hasn't run
    # migrations yet still gets a usable schema. Seed data is loaded
    # out-of-band via the SQL files in db/sql/ against the Neon/Postgres
    # database — the app no longer imports the pickle cache at startup.
    init_db()
